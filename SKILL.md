...
metadata:
  ...
  requires:
    ...
    env:
      - "GUAIKEI_API_TOKEN"
  env_desc:
    GUAIKEI_API_TOKEN: "快手数据 API 访问令牌。未配置时无法调用接口；可通过 https://www.guaikei.com 开通。"
  category:
    - "Integrations"
    - "Research"
    - "数据分析"
...
> ✨ **一句话价值**：查询和分析快手视频、达人、评论数据，筛选快手带货达人，结合达人互动数据、作品表现数据和带货效果给出合作优先级，找到真正和品类、受众及目标匹配的快手达人候选。支持关键词搜索排序、达人作品获取、视频评论获取；用于快手、KuaiShou或Kwai 数据分析、市场调研、达人筛选、爆品、趋势、市场容量、竞争结构等数据形成经营判断时使用，依据快手真实返回的数据形成分析结论。
...
> **Note:** 请先通过 [快手带货达人筛选技能](https://www.guaikei.com) 开通TOKEN，配置环境变量 `GUAIKEI_API_TOKEN` 后才能正常运行。
